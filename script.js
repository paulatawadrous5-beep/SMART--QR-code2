...
(function () {
  ...
  var THEMES = {
    generic:   { name: 'Website',   style: 'rounded',  eye: 'rounded', fg: '#17212b', fg2: '#2f8f8f', mid: null,      bg: '#ffffff', caption: 'Scan me' },
  ...
  // Ready-made "Smart Designs" presets. "smart" is dynamic (see applyPreset) and
  // reuses the existing per-platform THEMES above; the rest are fixed looks built
  // entirely from settings the QR designer already supports.
  var PRESETS = {
    minimal:    { name: 'Minimal',    style: 'classic',  eye: 'square',  fg: '#17212B', fg2: '#2F8F8F', mid: null,      bg: '#ffffff', caption: 'Scan me' },
    social:     { name: 'Social',     style: 'gradient', eye: 'rounded', fg: '#833ab4', fg2: '#f56040', mid: '#d62976', bg: '#ffffff', caption: 'Follow us' },
    business:   { name: 'Business',  style: 'rounded',  eye: 'rounded', fg: '#176B6B', fg2: '#102A2A', mid: null,      bg: '#ffffff', caption: 'Scan to connect' },
    restaurant: { name: 'Restaurant', style: 'organic',  eye: 'leaf',    fg: '#a8501f', fg2: '#e8a15c', mid: null,      bg: '#fff8f0', caption: 'View our menu' },
    neon:       { name: 'Neon',      style: 'neon',     eye: 'circle',  fg: '#25f4ee', fg2: '#fe2c55', mid: null,      bg: '#0b0b12', caption: 'Scan me' }
  };
  ...
  function initApp() {
    ...
    function freshState() {
      return {
        url: '', host: '', detected: 'generic', themeId: 'generic', presetId: 'smart',
        style: 'rounded', eye: 'rounded',
        fg: '#17212b', fg2: '#2f8f8f', mid: null, bg: '#ffffff',
        size: 512, margin: 4,
    ...
    var presetChips = Array.prototype.slice.call(document.querySelectorAll('[data-preset]'));
    var recommendedBadge = $('recommendedBadge');
    ...
    // "Smart Designs": apply a group of existing settings in one go.
    // Presets are a starting point only — every field they touch stays editable afterwards.
    function applyPreset(id) {
      state.presetId = (id === 'smart' || PRESETS[id]) ? id : 'smart';
      if (state.presetId === 'smart') {
        applyTheme(state.detected);
      } else {
        var p = PRESETS[state.presetId];
        state.style = p.style; state.eye = p.eye;
        state.fg = p.fg; state.fg2 = p.fg2; state.mid = p.mid;
        state.bg = p.bg;
        state.caption = true; state.captionText = p.caption;
        if (state.logoBgAuto) state.logoBg = autoLogoBg(state.bg);
      }
      syncControls();
      render();
    ...
    function syncControls() {
      ...
      presetChips.forEach(function (b) { b.setAttribute('aria-pressed', String(b.getAttribute('data-preset') === state.presetId)); });
      if (recommendedBadge) recommendedBadge.hidden = !state.url;
    ...
    urlForm.addEventListener('submit', function (ev) {
      ...
      libsPromise.then(function (ok) {
        ...
        var platformChanged = detected !== state.detected;
        state.url = parsed.url; state.host = parsed.host; state.detected = detected;
        if (firstTime) state.presetId = 'smart';
        if (firstTime || (platformChanged && state.presetId === 'smart')) {
          applyTheme(detected);
        }
        studio.hidden = false;
        syncControls();
    ...
    themeSelect.addEventListener('change', function () {
      ...
      state.presetId = themeSelect.value === state.detected ? 'smart' : null;
      syncControls(); render();
    });
    ...
    presetChips.forEach(function (b) {
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { parseUrl: parseUrl, detectPlatform: detectPlatform, buildSvg: buildSvg, assess: assess, THEMES: THEMES, EYES: EYES, PRESETS: PRESETS };
  }
})();
